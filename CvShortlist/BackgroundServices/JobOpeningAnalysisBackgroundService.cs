...
public class JobOpeningAnalysisBackgroundService : BackgroundService
{
		...
		ConfigurationData configurationData)
	{
		...
		_waitTimeBetweenExecutions = TimeSpan.FromMinutes(_configurationData.JobAnalysisWaitTimeInMinutes);
	}
	...
	protected override async Task ExecuteAsync(CancellationToken stoppingToken)
	{
		while (!stoppingToken.IsCancellationRequested)
		{
			try
			{
				...
				var jobOpeningsInAnalysis = await GetJobOpeningsInAnalysis(dbExecutionService, stoppingToken);
				...
				foreach (var aJobOpeningInAnalysis in jobOpeningsInAnalysis)
				{
					...
					await UpdateProcessedJobOpening(dbExecutionService, aJobOpeningInAnalysis);
	...
	{
		var jobOpeningsInAnalysis = await dbExecutionService.ExecuteQueryAsync(async dbContext =>
		{
	...
	private async Task UpdateProcessedJobOpening(IDbExecutionService dbExecutionService, JobOpening processedJobOpening)
	{
		try
		{
			var currentDate = DateTime.UtcNow;
			var jobOpeningExpirationDate = currentDate.AddMonths(JobOpening.ExpirationInMonths);

			processedJobOpening.Status = JobOpeningStatus.AnalysisCompleted;
			processedJobOpening.DateLastModified = currentDate;
			processedJobOpening.DateOfExpiration = jobOpeningExpirationDate;
	...
	{
		var blobData = await _blobService.DownloadBlobData(candidateCv.BlobContainerName, candidateCv.BlobName);

		return new BinaryData(blobData);
	}
