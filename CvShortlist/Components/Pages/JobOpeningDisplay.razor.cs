...
public partial class JobOpeningDisplay : ComponentBase
{
    ...
    private const string MaximumPdfFileAdditionalUploadCountExceededOneFileLeftErrorMessage =
        "Cannot upload more than an additional 1 PDF file for the current job opening.";
    private const string MaximumPdfFileAdditionalUploadCountExceededMultipleFilesLeftErrorMessage =
        "Cannot upload more than an additional {0} PDF files for the current job opening.";
    ...
    private async Task DeleteJobOpening()
    {
        try
        {
            var confirmDeleteJobOpening = await ShowConfirmDialog(
                $"Are you sure you want to delete the current job opening?{NewLine}{NewLine}Deleted job openings are not returned to the subscription, and are lost forever.");
    ...
    private async Task StoreSelectedPdfFilesInBrowser()
    {
        ...
        else if (uploadPendingPdfFiles.Count > _jobOpening!.RemainingCandidateCvsAllowedToBeUploaded)
        {
            _uploadPendingPdfFiles = [];

            var maximumPdfFileAdditionalUploadCountExceededErrorMessage =
                GetMaximumPdfFileAdditionalUploadCountExceededErrorMessage();

            _pdfUploadMessages.Add(new ReadFileMessage(
                ReadFileMessageType.Error,
                maximumPdfFileAdditionalUploadCountExceededErrorMessage));
        }
        else
        {
    ...
    private async Task UploadPdfFiles()
    {
        ...
        else if (_uploadPendingPdfFiles!.Count > _jobOpening!.RemainingCandidateCvsAllowedToBeUploaded)
        {
            _uploadPendingPdfFiles = [];
    ...
    private string GetMaximumPdfFileAdditionalUploadCountExceededErrorMessage()
    {
        var remainingCandidateCvsAllowedToBeUploaded = _jobOpening!.RemainingCandidateCvsAllowedToBeUploaded;

        return remainingCandidateCvsAllowedToBeUploaded == 1
            ? MaximumPdfFileAdditionalUploadCountExceededOneFileLeftErrorMessage
            : string.Format(
                MaximumPdfFileAdditionalUploadCountExceededMultipleFilesLeftErrorMessage,
                remainingCandidateCvsAllowedToBeUploaded);
    }
